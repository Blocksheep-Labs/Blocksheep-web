...
module.exports = (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("disconnect", () => {
      ...
      const roomsToEmitDisconnectEvent = userConnection
        ? [userConnection.room]
        : [];
      ...
      if (userConnection) {
        ...
        const usersInRace = connectedUsers.filter(
          (u) => u.raceId == userConnection.raceId
        );
        if (usersInRace.length == 0) {
          ...
          const roomScreenIndex = roomsLatestScreen.findIndex(
            (i) => i.raceId == userConnection.raceId
          );
          if (roomScreenIndex == -1) {
            ...
            ["UNDERDOG", "BULL_RUN", "RABBIT_HOLE"].includes(
              roomsLatestScreen[roomScreenIndex].screen
            )
          ) {
        ...
        if (userConnection.userAddress && userConnection.raceId) {
          ...
          if (!activePlayers[userConnection.raceId])
            activePlayers[userConnection.raceId] = [];
          if (!waitingPlayers[userConnection.raceId])
            waitingPlayers[userConnection.raceId] = [];

          // Remove the disconnected user from both arrays
          activePlayers[userConnection.raceId] = activePlayers[
            userConnection.raceId
          ].filter((i) => i.userAddress != userConnection.userAddress);
          waitingPlayers[userConnection.raceId] = waitingPlayers[
            userConnection.raceId
          ].filter((i) => i.userAddress != userConnection.userAddress);
          ...
          if (!matchesPlayed[userConnection.raceId])
            matchesPlayed[userConnection.raceId] = {};
          if (!gameCompletesAmount[userConnection.raceId])
            gameCompletesAmount[userConnection.raceId] = 0;
          ...
          remainingPlayers.forEach((player) => {
            ...
            gamesRequired[userConnection.raceId][player.userAddress] =
              newRequiredGames;
            ...
            if (
              !matchesPlayed[userConnection.raceId][
                player.userAddress
              ]?.includes(userConnection.userAddress)
            ) {
              gameCounts[userConnection.raceId][player.userAddress] =
                (gameCounts[userConnection.raceId][player.userAddress] || 0) +
                1;
            }
    ...
    socket.on("connect-live-game", ({ raceId, userAddress, part }) => {
      ...
      connectedUsers = connectedUsers.filter(
        (user) => !(user.userAddress === userAddress && user.room === roomName)
      );
      ...
      if (
        roomScreenData &&
        screens.indexOf(roomScreenData.screen) < screens.indexOf(part)
      ) {
        console.log({ part });
    ...
    socket.on("minimize-live-game", ({ part, raceId }) => {
      ...
      const roomsToEmitDisconnectEvent = connectedUsers
    ...
    socket.on(
      "update-progress",
      ({ raceId, userAddress, property, value, version }) => {
        // console.log({ raceId, userAddress, property, value, version })
        const roomName = `race-${raceId}`;

        // try to find existing progress
        let rProgress = racesProgresses.find(
          (i) => i?.room === roomName && i?.userAddress === userAddress
        );
        ...
        if (property === "game2-eliminate") {
          ...
          racesProgresses.forEach((progress) => {
            ...
            if (
              progress.userAddress !== userAddress &&
              !connectedUsers.some(
                (user) => user.userAddress === progress.userAddress
              )
            ) {
          ...
          const eliminatingPlayerProgress = racesProgresses.find(
            (progress) => progress.userAddress === userAddress
          );
        ...
        const index = racesProgresses.findIndex(
          (i) => i?.room === roomName && i?.userAddress === userAddress
        );
        racesProgresses[index] = updatedProgress;

        io.to(roomName).emit("progress-updated", {
          raceId,
          property,
          value,
          userAddress,
          rProgress: updatedProgress,
        });
      }
    );
    ...
    socket.on(
      "set-questions-state",
      ({ raceId, newIndex, secondsLeft, state }) => {
        const roomName = `race-${raceId}`;
        ...
        const newStateLevel = questionsGameStates.indexOf((i) =>
          i.includes(state)
        );
        const currStateLevel = questionsGameStates.indexOf((i) =>
          i.includes(currData.state)
        );
    ...
    socket.on(
      "set-tunnel-state",
      ({ raceId, secondsLeft, addRoundsPlayed, gameState, isFinished }) => {
    ...
    socket.on("get-progress", ({ raceId, userAddress }) => {
      const roomName = `race-${raceId}`;
      const progress = racesProgresses.find(
        (i) => i?.room === roomName && i?.userAddress === userAddress
      );
    ...
    socket.on("tunnel-started", ({ raceId }) => {
      const roomName = `race-${raceId}`;
      io.to(roomName).emit("tunnel-started-on-client", {
        socketId: socket.id,
        raceId,
      });
    });
    ...
    socket.on("rabbit-hole-results-shown", ({ raceId }) => {
      const roomName = `race-${raceId}`;
      io.to(roomName).emit("rabbit-hole-results-shown-on-client", {
        socketId: socket.id,
        raceId,
      });
    });
    ...
    socket.on("underdog-results-shown", ({ raceId }) => {
      const roomName = `race-${raceId}`;
      io.to(roomName).emit("underdog-results-shown-on-client", {
        socketId: socket.id,
        raceId,
      });
    });
    ...
    socket.on("get-connected", ({ raceId }) => {
      const roomName = `race-${raceId}`;
      const connectedInRoom = connectedUsers.filter(
    ...
    socket.on("bullrun-win-modal-opened", async ({ raceId }) => {
      const roomName = `race-${raceId}`;
      io.to(roomName).emit("bullrun-win-modal-opened-on-client", {
        raceId,
        socketId: socket.id,
      });
    });
    ...
    socket.on(
      "bullrun-join-game",
      async ({ raceId, userAddress, amountOfGamesRequired }) => {
        ...
        function initializePlayer(playerAddress, socket) {
          if (!matchesPlayed[raceId][playerAddress])
            matchesPlayed[raceId][playerAddress] = [];
          if (!gameCounts[raceId][playerAddress])
            gameCounts[raceId][playerAddress] = 0;
          if (!gamesRequired[raceId][playerAddress])
            gamesRequired[raceId][playerAddress] = amountOfGamesRequired;

          if (!inGamePlayers[raceId]) inGamePlayers[raceId] = [];

          // Check if the player is not in inGamePlayers
          if (
            !Array(inGamePlayers[raceId]).indexOf(
              (i) => i.userAddress == playerAddress
            )
          ) {
        ...
        function pairPlayers() {
          ...
          while (activePlayers[raceId].length >= 2 && retries < maxRetries) {
            ...
            if (
              gameCounts[raceId][player1.userAddress] <
                gamesRequired[raceId][player1.userAddress] &&
              gameCounts[raceId][player2.userAddress] <
                gamesRequired[raceId][player2.userAddress] &&
              !matchesPlayed[raceId][player1.userAddress].includes(
                player2.userAddress
              ) &&
              !matchesPlayed[raceId][player2.userAddress].includes(
                player1.userAddress
              )
            ) {
              ...
              matchesPlayed[raceId][player1.userAddress].push(
                player2.userAddress
              );
              matchesPlayed[raceId][player2.userAddress].push(
                player1.userAddress
              );
          ...
          if (activePlayers[raceId].length === 1) {
            ...
            if (
              gameCounts[raceId][remainingPlayer.userAddress] >=
              gamesRequired[raceId][remainingPlayer.userAddress]
            ) {
            ...
            io.to(remainingPlayer.id).emit("bullrun-waiting", {
              message: `Waiting for an opponent, games played: ${gamesPlayed}`,
              raceId,
            });
          }
    ...
    socket.on("bullrun-game-end", ({ raceId }) => {
      ...
      if (
        gameCounts[raceId][socket.userAddress] >=
        gamesRequired[raceId][socket.userAddress]
      ) {
    ...
    socket.on("race-finish", ({ raceId, userAddress }) => {
      ...
      entries.forEach((i, key) => {
        ...
        console.log(i[0], property, raceId);

        finishRace(i[0], property, raceId);
      });
    });
  });
